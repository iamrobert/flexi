...
if ($this->ordering) {
	$image_ordering_tip = '<i class="icon-warning color-warning fc-man-icon-s '.$tip_class.'" alt="Reordering" title="'.flexicontent_html::getToolTip('FLEXI_REORDERING', 'FLEXI_REORDERING_ENABLED_DESC', 1, 1).'"></i> ';
	$drag_handle_box = '<div class="fc_drag_handle%s" title="'.JText::_('FLEXI_ORDER_SAVE_WHEN_DONE', true).'"></div>';
} else {
	
	
	$image_ordering_tip = '<i class="icon-info hasTooltip purple" title="'.flexicontent_html::getToolTip('FLEXI_REORDERING', 'FLEXI_REORDERING_DISABLED_DESC', 1, 1).'"></i>';
  ...
  <form action="index.php?option=<?php echo $this->option; ?>&view=<?php echo $this->view; ?>" method="post" name="adminForm" id="adminForm" class="form-horizontal">
    ...
    <div id="j-main-container">
      ...
      <div id="fc-filters-box" <?php if (!$this->count_filters) echo 'style="display:none;"'; ?> class=""> 
        ...
        <span class="fc-filter nowrap_box"> <?php echo $this->lists['filter_type']; ?> </span> <span class="fc-filter nowrap_box"> <?php echo $this->lists['assigned']; ?> </span> <span class="fc-filter nowrap_box"> <?php echo $this->lists['fftype']; ?> </span> <span class="fc-filter nowrap_box"> <?php echo $this->lists['state']; ?> </span> <span class="fc-filter nowrap_box"> <?php echo $this->lists['access']; ?> </span>
      ...
      <span style="display:none;" class="alert fc-small fc-iblock" id="fcorder_save_warn_box"> <?php echo JText::_('FLEXI_FCORDER_CLICK_TO_SAVE') .' '. ($this->ordering ? JHTML::_('grid.order', $this->rows, 'filesave.png', $ctrl.'saveorder' ) : '') ; ?> </span>
      
      <div class="block-flat">
      <table id="adminListTableFCfields" class="adminlist fcmanlist table no-border hover">
        <thead>
          <tr class="header">
            ...
            <th class="center hidden-tablet hidden-phone"> <?php echo $image_ordering_tip; ?>
            ...
            <th class="hideOnDemandClass title left"><?php echo JHTML::_('grid.sort', 'FLEXI_FIELD_LABEL', 't.label', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
            <th class="hideOnDemandClass left hidden-tablet hidden-phone"><?php echo JHTML::_('grid.sort', 'FLEXI_FIELD_NAME', 't.name', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
            <th class="hideOnDemandClass"><?php echo JHTML::_('grid.sort', 'FLEXI_FIELD_TYPE', 't.field_type', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
            <th class="hideOnDemandClass center hidden-tablet hidden-phone" nowrap="nowrap"> <?php echo '<small class="badge">'.JText::_( 'Content Lists' ).'</small>'; ?><br/>
              <small> <?php echo JHTML::_('grid.sort', 'FLEXI_FIELD_CONTENT_LIST_TEXT_SEARCHABLE', 't.issearch', $this->lists['order_Dir'], $this->lists['order'] ); ?> / <?php echo JHTML::_('grid.sort', 'FLEXI_FIELD_CONTENT_LIST_FILTERABLE', 't.isfilter', $this->lists['order_Dir'], $this->lists['order'] ); ?> </small> <span class="column_toggle_lbl" style="display:none;"><?php echo '<small class="badge">'.JText::_( 'Content Lists' ).'</small>'; ?></span> </th>
            <th class="hideOnDemandClass center hidden-tablet hidden-phone" nowrap="nowrap"> <?php echo '<small class="badge">'.JText::_( 'Search view' ).'</small>'; ?><br/>
              <small> <?php echo JHTML::_('grid.sort', 'FLEXI_FIELD_ADVANCED_TEXT_SEARCHABLE', 't.isadvsearch', $this->lists['order_Dir'], $this->lists['order'] ); ?> / <?php echo JHTML::_('grid.sort', 'FLEXI_FIELD_ADVANCED_FILTERABLE', 't.isadvfilter', $this->lists['order_Dir'], $this->lists['order'] ); ?> </small> <span class="column_toggle_lbl" style="display:none;"><?php echo '<small class="badge">'.JText::_( 'Search view' ).'</small>'; ?></span> </th>
            <th class="hideOnDemandClass center hidden-phone" colspan="2"><?php echo JHTML::_('grid.sort', 'FLEXI_ASSIGNED_TYPES', 'nrassigned', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
		...
		for ($i=0, $n=count($this->rows); $i < $n; $i++)
		{
          ...
          <tr class="<?php echo "row$k"; ?>" style="<?php echo $row_css; ?>">
            ...
            <td class="order hidden-tablet hidden-phone"><?php
					$show_orderUp   = $i > 0;
            ...
            <td class="center"><?php
				...
				?></td>
            <?php endif; ?>
            <td class="left hidden-tablet hidden-phone"><?php
				...
				if ($field_desc) echo ' <i class="icon-info" title="'.flexicontent_html::getToolTip($_desc_label, $field_desc, 0, 0).'"></i>';
				?></td>
            <td align="left"><?php
				if (isset($row->grouping_field)) {
					...
					<a style="padding:2px;" href="'.$_link.'" title="'.$edit_entry.'">
						...
						<i class="icon-warning color-warning fc-man-icon-s '.$tip_class.'" alt="Reordering" title="Grouped inside: '.htmlspecialchars($_r->label, ENT_QUOTES, 'UTF-8').'"></i>
					</a>';
				}
				...
				if ($row->checked_out) {
					...
					if ($canCheckin) {
						...
						?>
              <a class="jgrid <?php echo $tip_class; ?>" title="<?php echo $_tip_title; ?>" href="javascript:;" onclick="var ccb=document.getElementById('cb<?php echo $i;?>'); ccb.checked=1; ccb.form.task.value='<?php echo $task_str; ?>'; ccb.form.submit();"> <img src="components/com_flexicontent/assets/images/lock_delete.png" alt="Check-in" class="fc-man-icon-s" /> </a>
              <?php
					} else {
				...
				?>
              <a href="<?php echo $link; ?>" title="<?php echo $edit_entry; ?>"> <?php echo htmlspecialchars(JText::_($row->label), ENT_QUOTES, 'UTF-8'); ?> </a> <?php echo $original_label_text;?>
              <?php
            ...
            <td class="text-left"><?php
				...
				?></td>
            <td class="center hidden-tablet hidden-phone"><?php if($supportsearch) :?>
              <?php if ($canEdit) :?>
              ...
              <span style="display:inline-block; width:16px; height:16px;"></span>
              <?php endif; ?></td>
            <td class="center hidden-tablet hidden-phone"><?php if($supportadvsearch) :?>
              <?php if ($canEdit) :?>
              ...
              <span style="display:inline-block; width:16px; height:16px;"></span>
              <?php endif; ?></td>
            <td class="center hidden-phone"><?php echo $row->nrassigned ? '<span class="badge badge-info hasTooltip">'.$row->nrassigned.'</span>' : $orphan_warning; ?></td>
