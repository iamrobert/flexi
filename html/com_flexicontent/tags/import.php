...
window.addEvent('domready', function(){
	$('adminForm').addEvent('submit', function(e) {
		...
		if (MooTools.version>="1.2.4") {
			$('log-bind').set('html','<p class="centerimg"><img src="templates/flexi/images/flexi/ajax-loader.gif" class="center"></p>');
			new Request.HTML({
				 url: this.get('action'),
			   evalScripts: true,
			   update: $('log-bind'),
			...
			$('log-bind').setHTML('<p class="centerimg"><img src="templates/flexi/images/flexi/ajax-loader.gif" class="center"></p>');
			this.send({
				update: 	$('log-bind')
			});
		}
...
<form action="index.php?option=com_flexicontent&".$ctrl_task."import&layout=import&<?php echo FLEXI_J16GE ? 'format=raw' : 'tmpl=component';?>" method="post" name="adminForm" id="adminForm">
	...
	<fieldset>
		<legend style="font-size:12px; font-style:arial;" >
			...
			<img class="editlinktip hasTip tags" title="<?php echo JText::_( 'FLEXI_IMPORT_TAGS_DESC' ); ?>" style="float:none; margin:0 0 -4px 4px;" src="templates/flexi/images/flexi/information.png" border="0" alt="Note"/>
		</legend>
